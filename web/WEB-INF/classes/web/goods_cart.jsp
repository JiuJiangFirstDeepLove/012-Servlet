		...
		<div class="container">
			...
			<c:forEach items="${cartList }" var="item">
				<div class="cart-header col-md-6 cart-data" data-price="${item.price}" data-amount="${item.amount}">
					<div class="cart-sec simpleCart_shelfItem">
						<div class="cart-item cyc">
							<a href="/goods_detail?id=${item.goodsId}">
								<img src="${pageContext.request.contextPath }${item.cover}" class="img-responsive">
							</a>
						</div>
						<div class="cart-item-info">
							<h3><a href="/goods_detail?id=${item.goodsId}">${item.name}</a></h3>
							<h3><span>单价: ¥ ${item.price}</span></h3>
							<h3><span>数量: ${item.amount}</span></h3>
							<a class="btn btn-info" href="javascript:buy(${item.goodsId});">增加</a>
							<a class="btn btn-warning" href="javascript:lessen(${item.goodsId});">减少</a>
							<a class="btn btn-danger" href="javascript:deletes(${item.goodsId});">删除</a>
						</div>
						<div class="clearfix"></div>
					</div>
				</div>
