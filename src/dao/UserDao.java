...
public class UserDao {
    public void addUser(User user) throws SQLException {
        ...
        String sql = "insert into user(username,email,password,name,phone,address,isadmin,isvalidate,user_name,photo) values(?,?,?,?,?,?,?,?,?,?)";
        r.update(sql,user.getUsername(),user.getEmail(),user.getPassword(),user.getName(),user.getPhone(),user.getAddress(),user.isIsadmin(),user.isIsvalidate(),user.getUser_name(),user.getPhoto());
    }
    ...
    public void updateUserAddress(User user) throws SQLException {
        QueryRunner r = new QueryRunner(DataSourceUtils.getDataSource());
        String sql ="update user set name = ?,phone=?,address=?,photo=?,user_name=? where id = ?";
        r.update(sql,user.getName(),user.getPhone(),user.getAddress(),user.getPhoto(),user.getUser_name(),user.getId());
    }
